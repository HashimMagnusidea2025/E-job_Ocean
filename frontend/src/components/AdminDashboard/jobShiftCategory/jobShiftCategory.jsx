...
export default function JobShiftCategoryPage() {
  ...
  const fetchCategories = async () => {
    try {
      const res = await axios.get("/job-Shift-category");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await axios.put(`/job-Shift-category/${formData.id}`, {
          name: formData.name,
          status: formData.status
        });
        Swal.fire("Updated!", "Category updated successfully", "success");
      } else {
        await axios.post("/job-Shift-category", {
          name: formData.name,
          status: formData.status
        });
  ...
  const handleDelete = async (id) => {
    ...
    if (confirm.isConfirmed) {
      try {
        await axios.delete(`/job-Shift-category/${id}`);
      ...
      cell: ({ row }) => (
        <div className="flex gap-3">
          ...
          <button
            ...
            className={row.original.status === "active" ? "text-blue-600" : "text-gray-600"}
          >
    ...
    <Layout>
      <div className="p-6 max-w-6xl mx-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-gray-800">Job Shift Categories</h2>
          <input
            type="text"
