...
export default function AdminOneToOneForm() {
    ...
    // Error states for each form
    const [errors, setErrors] = useState([{}]);

    // Handle input change for specific form index
    ...
    const validateForm = () => {
        const newErrors = forms.map((form, index) => {
            ...
            if (!form.Speaker) formErrors.Speaker = "Speaker is required";
            ...
            if (!form.fees) formErrors.fees = "Fees is required";
        ...
        try {
            if (editId) {
                ...
                for (let formData of forms) {
                    await axios.post("/one-to-one", formData);
                }
                alert("Session(s) created successfully!");
            }
            navigate("/admin-dashboard/one-to-one-list");
        } catch (err) {
            console.error(err);
            ...
            <div className="max-w-3xl mx-auto p-6 bg-white rounded-2xl shadow-lg font-[Poppins]">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-6">
                        ...
                        <div key={index} className="border p-4 rounded-lg mb-6 relative">
                            ...
                            <div className="flex flex-col mb-2">
                                ...
                                <select
                                    ...
                                    className={`border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 ${errors[index]?.Speaker ? "border-red-500" : "border-gray-300"
                                        }`}
                                >
                                ...
                                {errors[index]?.Speaker && (
                                    <p className="text-red-500 text-sm mt-1">{errors[index].Speaker}</p>
                                )}
                            </div>
                            ...
                            {/* Fees + Including GST → Only show when Paid */}
                            ...
                            <div className="flex flex-col mb-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => {
                                        ...
                                        if (formData.startTime && end <= formData.startTime) {
                                            setErrors(prev => ({
                                                ...prev,
                                                [index]: { ...prev[index], endTime: "End Time must be later than Start Time" },
                                            }));
                                        } else {
                                            setErrors(prev => ({
                                                ...prev,
                                                [index]: { ...prev[index], endTime: "" },
                                            }));
                                        }
                                    }}
                    ...
                    <div className="flex justify-end gap-4 pt-4 border-t">
                        <button
                            type="button"
                            onClick={() => navigate("/admin-dashboard/one-to-one-list")}
