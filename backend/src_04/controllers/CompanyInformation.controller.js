...
export default class CompanyInformationController {
  ...
  static async CompanyInformationcreate(req, res) {
    try {
      ...
      // if (req.file) {
      //   company.hiringcompanies = `/uploads/hiringCompanies/${req.file.filename}`;
      // }
      // ✅ Handle employerLogo and hiringCompanies separately
      if (req.files?.employerLogo?.[0]) {
        company.employerLogo = `/uploads/employerLogo/${req.files.employerLogo[0].filename}`;
      }

      if (req.files?.hiringCompanies?.[0]) {
        company.hiringcompanies = `/uploads/hiringCompanies/${req.files.hiringCompanies[0].filename}`;
      }
  ...
  static async CompanyInformationupdate(req, res) {
    try {
      ...
      //   updateData.company.hiringcompanies = `/uploads/hiringCompanies/${req.file.filename}`
      ...
      //   updateData.company.hiringcompanies = req.body.existingLogo;
      // } else {
      //   // Keep the existing logo from database
      //   updateData.company.employerLogo = existingCompany.company.employerLogo;
      //   updateData.company.hiringcompanies = existingCompany.company.hiringcompanies;
      // }


      // ✅ Handle uploaded files separately
      if (req.files?.employerLogo?.[0]) {
        updateData.company.employerLogo = `/uploads/employerLogo/${req.files.employerLogo[0].filename}`;
      } else if (req.body.existingLogo) {
        updateData.company.employerLogo = req.body.existingLogo;
      } else {
        updateData.company.employerLogo = existingCompany.company.employerLogo;
      ...
      if (req.files?.hiringCompanies?.[0]) {
        updateData.company.hiringcompanies = `/uploads/hiringCompanies/${req.files.hiringCompanies[0].filename}`;
      } else if (req.body.existingHiringLogo) {
        updateData.company.hiringcompanies = req.body.existingHiringLogo;
      } else {
        updateData.company.hiringcompanies = existingCompany.company.hiringcompanies;
      }
